...
import br.edu.ufcg.computacao.psoft.commerce.service.produtoService.ProdutoValidarCodigoService;
...
class ProdutoV1RestControllerTest {
    ...
    ProdutoValidarCodigoService produtoValidarCodigoService;
    ...
    @Test
    @DisplayName("Quando buscar produto por ID inexistente, então deve retornar ProdutoNaoExisteException")
    public void quandoBuscarProdutoPorIdInexistente() throws Exception {
        Long idInexistente = 99989999L;
        // Act & Assert
        driver.perform(MockMvcRequestBuilders.get("/v1/produto/" + idInexistente)
                        .contentType(MediaType.APPLICATION_JSON))
                ...
                .andExpect(result -> {
                    Throwable throwable = result.getResolvedException();
                    assertNotNull(throwable);
                    assertTrue(throwable instanceof ProdutoNaoExisteException);
                    assertEquals("O produto não existe!", throwable.getMessage());
                });
